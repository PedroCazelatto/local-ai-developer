...
import { parseReviewIssue } from './parse-review-issue.js'; // one issues[] entry, or the reason it is not one
...
export function parseVerdict(args: Record<string, unknown>): VerdictParse {
  ...
  if (result !== 'pass' && result !== 'fail') {
    return parseFailure(`"result" must be "pass" or "fail" (got ${describeValue(result)}).`);
  }
  const summary = args['summary'];
  if (typeof summary !== 'string' || summary.trim() === '') {
    return parseFailure('"summary" must be a non-empty string describing the overall judgment.');
  }
  const rawIssues = args['issues'] ?? [];
  if (!Array.isArray(rawIssues)) {
    return parseFailure('"issues" must be an array (use [] when there are none).');
  }

  const issues: ReviewIssue[] = [];
  for (let i = 0; i < rawIssues.length; i += 1) {
    const parsed = parseReviewIssue(rawIssues[i], i);
  ...
  if (result === 'pass') {
    ...
    return parseFailure('result is "fail" but "issues" is empty. A fail must list at least one concrete issue.');
  }
