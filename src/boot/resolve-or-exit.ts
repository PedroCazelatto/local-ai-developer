...
export function resolveOrExit(projectName: string): SessionConfig {
  try {
    // loadConfig resolves the env-backed session settings and validates that the project directory is
    // really there, throwing rather than returning a half-built config when it is not.
