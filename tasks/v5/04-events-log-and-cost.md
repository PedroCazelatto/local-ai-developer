> **Status:** ✅ Completed (2026-07-12) — Orchestrator events log at `projects/<active>/.orchestrator/events.jsonl`, an append-only **sibling** of the tool-audit log written through a shared `append-jsonl-line.ts` writer (extracted from `audit.ts`; fsync-per-line, so both logs share one durability path). `events-log.ts` + `events-log.type.ts` emit six typed events (no `any`; `ts` stamped at write, absent optionals omitted — never null placeholders): `phase_swap {from,to}` and `memory_load {phase,turnsLoaded}` + exact restored `prompt_eval_count` (from `switchPhase`/constructor — `activatePhase` now returns a `PhaseLoad`, firing only on a first-time disk load with turns), `summarization_fire` with **exact** before/after prompt counts (deferred: `beforeModelCall` stores the pre-compaction size, the next `onTokens` supplies the post-compaction `after`; a missing metric → `incomplete:true`, never estimated), `subagent_spawn`/`subagent_dismiss` (from `SubagentManager`, carrying `subagent_id` + initial exact tokens), and `model_use {from,to}` (from `useModel`). The log is never injected into any prompt. Cost visibility: exact cumulative per-phase token total accumulates in the orchestrator via `addTokenCounts` (null-poisoned) and renders on the status line as a compact `Σ` (`Σ?` when incomplete, omitted before the phase's first turn); per-sub-agent totals already surface via `/subagents` (V5/01). Error pass: swept every tool — all already return structured recoverable `{error,…}`, the dispatcher converts any residual throw, and `repl.ts` catches a dropped Ollama stream, so the turn + REPL survive a tool failure or a dead daemon with **no code change** needed. Typecheck clean + events-log JSONL shape verified with a throwaway script; full live before/after + restart acceptance needs a running Ollama + sandbox.
