> **Status:** ✅ Completed (2026-07-12) — the model is de-hardcoded into a **mutable live field** on `OllamaClient` (`setModel` / `get model`); `chat`/`stream` keep their signature, so every window — phase turns, spawned Worker/Reviewer/Retro, sub-agents, `oneShot` — shares the ONE live model. Per the user's directive there is **no per-window pinning**: all windows always use the current model, which only ever changes between turns (the single-threaded REPL is blocked during any turn/batch). Sub-agents drop the static `deps.model` and record the live model at spawn (reference only). Boot resolves the model **state.json → `DEFAULT_MODEL`**; the choice persists to `~/.local-ai-developer/state.json` (home-dir, orchestrator-**global**, never per-project) via `core/session/app-state.ts` (temp-then-rename write; missing = normal, corrupt = warn + fall back, never crashes boot). Per the user's choice the command hangs off a **new static command registry** (`interface/command-registry.ts`, mirroring `tools/registry.ts`, carrying `summary`/`usage` for V5/03's `/help`); `repl.ts` dispatches registry-first and falls back to the switch for un-migrated commands. `commands/models.ts` implements `list` (marks the active model), `pull` (ora-streamed, **blocks to completion**, Ctrl-C aborts ONLY the pull via a readline `SIGINT` → `AbortController` bridged onto the ollama stream iterator — REPL survives), and `use` (`hasModel`-guarded against an un-pulled name, applies via `orchestrator.useModel`, then persists). Daemon wrappers `core/llm/ollama-models.ts` (`listModels`/`hasModel`/`pullModel`) talk to the host Ollama directly (not sandboxed). Typecheck clean; live acceptance (list/pull/Ctrl-C/use/persist + corrupt-state fallback) needs a running Ollama daemon.
