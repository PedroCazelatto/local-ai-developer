> **Status:** ✅ Completed (2026-07-12) — Status line gains four fields on two pinned bottom rows (DECSTBM scroll region — scrollback stays intact, not the alt-buffer): a persistent color-coded active phase (per-phase colors in `theme.ts`), a live `running <tool> (X.Xs)` current-tool timer, a `thinking`/`Xs since last tool` stall indicator, and a conditional `Subagents: N` (dropped at zero). Live timers live in `core/ui/status-activity.ts` — UI-only, wired from the turn loop's tool-dispatch bracket, never feeding any VRAM/summarization decision; token totals stay exact (unknown → `?`, never estimated). A second pinned footer row shows the `Shift+Tab: cycle phase · /swap <phase>: jump · /help: commands` hint; Shift+Tab (back-tab) cycles the active phase, guarded to idle so it can't switch a phase mid-turn. Every command migrated into `interface/command-registry.ts` (name/group/description/usage + duplicate-name guard) — the REPL fallback switch is gone and `/exit` signals via `requestExit`; new `commands/{exit,help,swap}.ts`. `/help` (`commands/help.ts`) is auto-generated from `listCommands()`, grouped by purpose, so a new command appears with no hand-maintained list. Typecheck clean; live status-line / `/help` / Shift+Tab acceptance needs a running `run start` session.
