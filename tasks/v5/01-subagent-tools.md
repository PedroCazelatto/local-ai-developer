> **Status:** ✅ Completed (2026-07-11) — `core/session/subagents.ts` (`SubagentManager`, owned by the orchestrator; in-memory `Map`, dies with the session) + `subagents.type.ts` + `generate-subagent-id.ts` (26-char ULID-shaped id, short = first 4). Three global tools `tools/{spawn,ask,dismiss}-subagent.ts` delegate through `ctx.subagents` (a new optional `SubagentHandle` on `ToolContext`, wired only by the orchestrator for the interactive master phases). A sub-agent runs its own bounded tool-dispatch loop **non-streaming** — only its `→ tool: <name> [sub:<short>]` markers print (user chose quiet over full streaming), and its own calls are audited with an extra `subagent_id` (`phase` = the master phase). Its tool defs are the master's set **minus** the three sub-agent tools (no nesting, by construction) — the orchestrator likewise hands the spawned **execution** windows (Worker/Reviewer/Retro) a sub-agent-tool-free set, so only interactive phases can spawn this task. `ask` on an unknown/dismissed id returns a flat recoverable `{ error, code: "unknown_subagent" }` (codebase convention, not the task's nested example); `dismiss` is idempotent. Exact per-sub-agent `prompt/eval` totals accumulate via `addTokenCounts` (null-poisoned, never estimated); `/subagents` lists id·age·msgs·exact tokens and the status line shows `Subagents: N` while any are active. Typecheck clean; registry/filter/id/dismiss verified statically — live spawn→ask→dismiss needs a running Ollama + sandbox.
