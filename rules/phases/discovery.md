...
- **Never fake the question UI in text.** Do not draw a question in prose with horizontal rules (`───`), a checkbox list (`[ ] Yes` / `[ ] No`), or a "Please respond" line — the terminal prints those as dead characters the user cannot act on, and nothing you type is interactive. Even a simple yes/no validation is an `ask_user` call (e.g. options `["Yes, proceed", "No, let's revise"]`), never a hand-drawn menu. The **Validate before advancing** step above means an `ask_user` confirmation, not a paragraph.
